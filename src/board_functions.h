...
struct piece_attack_defend_struct {
    ...
    // define array maximum sizes
    constexpr static int arr_size = 63; // attack me includes all square that attack me
    constexpr static int view_size = 63;

    // four key lists for piece analysis
    std::array<StartDestMod, arr_size> attack_list_arr;
    std::array<StartDestMod, arr_size> attack_me_arr;
    std::array<StartDestMod, arr_size> defend_me_arr;
    std::array<int, view_size> piece_view_arr;

    // counters for the above arrays
    int attack_list_ind = 0;
    int attack_me_ind = 0;
    int defend_me_ind = 0;
    int piece_view_ind = 0;
    ...
    void attack_list_push_back(int dest_sq, int sq_value, int mod) {
        if (attack_list_ind >= arr_size) {
            throw std::runtime_error("attack_list_push_back arr_size exceeded");
        }
        attack_list_arr[attack_list_ind].dest_sq = dest_sq;
        attack_list_arr[attack_list_ind].sq_value = sq_value;
        attack_list_arr[attack_list_ind].mod = mod;
        attack_list_ind += 1;
    }

    void attack_me_push_back(int dest_sq, int sq_value, int mod) {
        if (attack_me_ind >= arr_size) {
            throw std::runtime_error("attack_me_push_back arr_size exceeded");
        }
        attack_me_arr[attack_me_ind].dest_sq = dest_sq;
        attack_me_arr[attack_me_ind].sq_value = sq_value;
        attack_me_arr[attack_me_ind].mod = mod;
        attack_me_ind += 1;
    }

    void defend_me_push_back(int dest_sq, int sq_value, int mod) {
        if (defend_me_ind >= arr_size) {
            throw std::runtime_error("defend_me_push_back arr_size exceeded");
        }
        defend_me_arr[defend_me_ind].dest_sq = dest_sq;
        defend_me_arr[defend_me_ind].sq_value = sq_value;
        defend_me_arr[defend_me_ind].mod = mod;
        defend_me_ind += 1;
    }

    void piece_view_push_back(int sq) {
        if (piece_view_ind >= view_size) {
            throw std::runtime_error("piece_view_push_back arr_size exceeded");
        }
        piece_view_arr[piece_view_ind] = sq;
        piece_view_ind += 1;
    }
...
struct total_legal_moves_struct {
    ...
    // create a pad struct for every square in the board
    std::array<piece_attack_defend_struct, 64> square_info;
